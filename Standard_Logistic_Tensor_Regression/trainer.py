...
class Net(nn.Module):
    def __init__(self,n_classes,code_dim = 100,activation = torch.nn.RReLU(),scaler_dim = 4,code_num = 2000):
        ...
        self.code_weight = Parameter(torch.Tensor(self.code_num,1))
        self.w = Parameter(torch.Tensor(self.code_dim + self.scaler_dim,2))
    def forward(self,x,x_scaler = None,x_time = None):
        """
        x is a tensor : 64 patient * 8438 code * 100 dim
        x_scaler: 64 * 4 
        
        """
        # alpha = torch.permute((torch.tanh(self.code_weight) + 1)/2,(1,0)) # 1 * 8000
        alpha = torch.permute(torch.tanh(self.code_weight),(1,0)) # 1 * 8000
        ...
        self.importance = alpha
        return x,alpha
    def reset_parameters(self):   
        nn.init.kaiming_uniform_(self.code_weight.data,nonlinearity='relu') 
...
class generic_LTR(BaseEstimator, ClassifierMixin): 
    def __init__(self, 
                 n_classes=2,
                 batch_size = 8,
                 weighted_loss = [1,1.5],
    ...
    def fit(self, X_train, X_scaler,X_time,y_train,code_num,norm_method = None):
        ...
        net = Net(self.n_classes,code_num = code_num)
        ...
        for global_step, (inputs, scaler, time, labels) in enumerate(train_loader, 0):
            ...
            out,alpha = net(inputs,scaler,time)
            ...
            elif norm_method == 'alpha':
                regu_loss = torch.norm(alpha,1)
            else:
                regu_loss = 0
    ...
    def predict(self, X, scaler, time):
        ...
        for i, (inputs, scaler, time) in enumerate(test_loader, 0):
            out,alpha = self.estimator(inputs, scaler, time)
    ...
    def predict_proba(self, X, scaler, time):
        ...
        weight_list = alpha.detach().numpy()
    ...
    def _model_testing(self, net, test_loader):
        ...
        for i, (inputs, scaler, time, labels) in enumerate(test_loader, 0):
            labels = labels.type(torch.long)
            x,alpha = net(inputs,scaler, time)
...
if __name__ == '__main__':
    ...
    classifier.fit(X_train, X_scaler, X_elapse,y_train,code_num,norm_method)
    ...
    # save model
    save_path = os.path.join('./models/','train_and_test')
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    pickle.dump(classifier, open(os.path.join(save_path, f'model_{exp_id}_bs_{batch_size}_lr{lr}_regu_{regu}_steps{max_steps}_timeaware_tanh_norm.mdl'), 'wb'))    
    with open(os.path.join(save_path,f'model_{exp_id}_bs_{batch_size}_lr{lr}_regu_{regu}_steps{max_steps}_loss_records.p'), 'wb') as f:
    ...
    np.save(os.path.join(weights_path,f'weight_{exp_id}_bs_{batch_size}_lr_{lr}_regu_{regu}_steps_{max_steps}_train_tanh_norm.npy'), train_weights)
